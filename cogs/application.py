...
class ApplicationModal(Modal, title="📝 Заявка в семью Ludoman clnx"):
    def __init__(self, *args, **kwargs):
        ...
        self.game_info = TextInput(
            label="Сколько уделяете игре и как узнали о нас",
            placeholder="Пример: Играю 4-5 часов в день. Узнал о вас от друга/из TikTok/с маркета",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Простая логика парсинга - пытаемся разделить по точкам или запятым
        parts = game_info.split('.')
        if len(parts) >= 2:
            playtime = parts[0].strip()
            discovery = '.'.join(parts[1:]).strip()
        else:
            # Если точки нет, пытаемся разделить по запятым
            parts = game_info.split(',')
            if len(parts) >= 2:
                playtime = parts[0].strip()
                discovery = ','.join(parts[1:]).strip()
            else:
                # Если вообще нет разделителей, считаем все за playtime
                playtime = game_info
...
Следование правилам```""",
        ...
        embed.add_field(
            name="📝 **ВОПРОСЫ В АНКЕТЕ:**",
            value="""1. Ваш игровой ник\n"""
                 """2. Ваш Static ID\n"""
                 """3. Ваш возраст (IRL)\n"""
                 """4. Ваше реальное имя\n"""
                 """5. **Сколько уделяете игре и как узнали о нас**""",
            inline=False
        )
